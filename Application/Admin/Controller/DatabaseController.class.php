...
class DatabaseController extends AdminController{
    ...
    public function optimize($tables = null){
        if($tables) {
            ...
            if(is_array($tables)){
                ...
                if($list){
					//记录行为
        			action_log('optimize_data',$tables,$tables,UID);
    ...
    public function repair($tables = null){
        if($tables) {
            ...
            if(is_array($tables)){
                ...
                if($list){
					//记录行为
        			action_log('repair_data',$tables,$tables,UID);
    ...
    public function del($time = 0){
        if($time){
			...
			//记录行为
        	action_log('delete_data','all',date('Ymd-His', $time),UID);
            if(count(glob($path))){
    ...
    public function export($tables = null, $id = null, $start = null){
        if(IS_POST && !empty($tables) && is_array($tables)){ //初始化
            ...
            if(false !== $Database->create()){
				...
				//记录行为
				action_log('backup_data','all',implode(",", $tables),UID);	
        ...
        if(is_numeric($time) && is_null($part) && is_null($start)){ //初始化
            ...
            if(false === $start){
                ...
                if(isset($list[++$part])){
					...
					//记录行为
        			action_log('restore_data','all',date('Ymd-His', $time),UID);	
                    $this->success('还原完成！');
                }
            } else {
