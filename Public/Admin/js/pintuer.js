$(function(){
	...
	$showtips=function(e,title){
		...
		if(place=="left"){
			...
			if($(document.body).outerWidth()>e.offset().left+html.outerWidth()){
				x=e.offset().left;
			}else{
				x=e.offset().left - html.outerWidth() - e.outerWidth();	
			}
			y=e.offset().top - html.outerHeight()/2 + e.outerHeight()/2;
		}
		...
		if (trigger=="hover" || trigger=="click" || trigger==null){
			//html.mouseout(function(){html.remove();e.attr("title",title);$(".dialog-mask").remove();});
			html.find(".tips-close,.close").each(function(){
				$(this).click(function(){
					html.remove();
					$('.dialog-mask').remove();
				});
